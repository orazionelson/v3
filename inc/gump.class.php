...
class GUMP
{
    ...
    */
    ...
    protected function validate_valid_name($field, $input, $param = null)
    {
		...
		$pattern="^([a-zÀÁÂÃÄÅÇÈÉÊËÌÍÎÏÒÓÔÕÖÙÚÛÜÝàáâãäåçèéêëìíîïñðòóôõöùúûüýÿ '-])+$";
        if (!preg_match("/".$pattern."/i", $input[$field]) !== false) {
            return array(
                'field' => $field,
                'value' => $input[$field],
                'rule' => __FUNCTION__,
   ...
   /* 
	 ...
	 */
	protected function validate_blacklist($field, $input, $param = NULL)
	    {
		...
		$badwords=Vcube::badwords();
		if(Vcube::strpos_array(strtolower($input[$field]), $badwords)>-1) {
			return array(
				'field' => $field,
				'value' => $input[$field],
				'rule' => __FUNCTION__,
	 ...
	 * 
	 **/
	protected function validate_captcha($field, $input, $param = NULL){
		...
		if(isset($_POST['sess_cap'])){
			$captcha=$_POST['sess_cap'];
			}
		else{
			$captcha=$_SESSION['captcha']['code'];
			}
		
		if ( $captcha != $input[$field]) {
		return array(
			'field' => $field,
			'value' => $input[$field],
			'rule' => __FUNCTION__,
